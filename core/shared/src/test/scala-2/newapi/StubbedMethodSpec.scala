...
class StubbedMethodSpec extends AnyFlatSpec with Matchers with Stubs {
  ...
  "StubbedMethod" should "set the return value for a method with arguments" in {
    ...
    (testStub.oneArg _).returns(x => s"Value: $x")
    testStub.oneArg(42) should be("Value: 42")
  }

  it should "track the number of times a method was called" in {
    val testStub = stub[TestTraitWithArgs]
    (testStub.oneArg _).returns(_ => "test")
    testStub.oneArg(1)
    testStub.oneArg(2)
    testStub.oneArg(3)
    (testStub.oneArg _).times should be(3)
  }
  ...
  it should "track the number of times a method was called with specific arguments" in {
    ...
    testStub.oneArg(2)
    testStub.oneArg(1)
    (testStub.oneArg _).times(1) should be(2)
    (testStub.oneArg _).times(2) should be(1)
    (testStub.oneArg _).times(3) should be(0)
  }
  ...
  it should "track the arguments with which a method was called" in {
    ...
    testStub.oneArg(2)
    testStub.oneArg(3)
    (testStub.oneArg _).calls should contain theSameElementsInOrderAs List(1, 2, 3)
  }

  it should "work with methods that have multiple arguments" in {
    val testStub = stub[TestTraitWithArgs]
    (testStub.twoArgs _).returns {
      case (1, "one") => 1
      case (2, "two") => 2
      case _ => 0
    }
  ...
  it should "set a constant return value with returnsWith" in {
    val testStub = stub[TestTraitWithArgs]
    (testStub.oneArg _).returnsWith("constant")
  ...
  it should "track call order with isBefore and isAfter" in {
    ...
    (testStub.oneArg _).returns(_ => "test")
    (testStub.twoArgs _).returns(_ => 42)

    testStub.oneArg(1)
    testStub.twoArgs(2, "two")

    (testStub.oneArg _).isBefore(testStub.twoArgs _) should be(true)
    (testStub.twoArgs _).isAfter(testStub.oneArg _) should be(true)
